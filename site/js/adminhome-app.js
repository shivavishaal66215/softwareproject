window.addEventListener("load", function () {
	...
	function process(event) {
		...
		if (
			event.clientX >= right_bound.x &&
			event.clientX <= right_bound.x + right_bound.width
		) {
			if (
				event.clientY >= right_bound.y &&
				event.clientY <= right_bound.y + right_bound.height
			) {
				...
				setTimeout(() => {
					location.href = `${"adminhomepage-" + dup.toLowerCase() + ".html"}`;
				}, 1000);
			...
			event.clientX >= left_bound.x &&
			event.clientX <= left_bound.x + left_bound.width
		) {
			if (
				event.clientY >= left_bound.y &&
				event.clientY <= left_bound.y + left_bound.height
			) {
				...
				setTimeout(function () {
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							document
								.getElementsByClassName("containers")[1]
								.classList.add("makebigger");
						}, 100);
					}, delayInMilliseconds);
				}, 600);
